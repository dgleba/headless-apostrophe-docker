...
module.exports = {
  ...
  construct(self, options) {
    // gzip files
    self.expressMiddleware = [(req, res, next) => compression()(req, res, next)]
     ...
     */
    self.apos.tasks.add('custom', 'webpack-dev-server', 'Start a webpack dev server', async function(
      apos,
      argv,
      callback,
    ) {
      ...
      console.info(
        colors.fg.green,
        'Webpack dev server ready on http://localhost:' + wpOptions.devServerPort,
        colors.reset,
      )
      console.info(
        colors.fg.grey,
