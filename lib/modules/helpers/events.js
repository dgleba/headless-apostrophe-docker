...
import { getData, getPreviousUrl, animateScripts, enterScripts, leaveScripts, widgets } from 'helpers'
...
const displayPage = (url, animate, options = {}) =>
  handleTransitionEnd(animate, options)
    .then(() => getData(url))
    .catch(console.log)

/**
 * Listen to end of transition/animation
 * @param {String} animate
 * @param {Object} options
 */
const handleTransitionEnd = (animate, options) =>
  new Promise(
    (resolve, reject) =>
      animate
        ? animateScripts.forEach(
            s =>
              s.type === animate &&
              s
                .script(options)
...
const popstate = e => {
  ...
  if (!window.apos) {
    ...
    if (menuBar) {
      ...
      const options = { number: menuBar.attributes.page.value }
