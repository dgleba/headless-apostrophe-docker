...
import { ajaxLoad, getData, pop, routes, widgets } from 'helpers'
...
const animateBeforeLeaving = (number, animate, navs) => {
  ...
  const atContentCareer = document.querySelector('.at_timeline')
  const menuBar = document.querySelector(`.${root}__${number}`)
  let wait = false

  if (atContent && !atContentHome && !atContentCareer) {
    document.querySelector('html').style = 'none' // remove styling put on previous page (i.e career pages)
...
const handleHrefClick = () => {
  const elements = document.querySelectorAll('[href].ajax')
...
const runScripts = () => {
  pages.forEach(page => {
    ...
    if (window.location.pathname.split('/')[1] === page.slug.slice(1)) {
      ...
      const attr = 'data-apos-widget'
      area && area.forEach(a => widgets.forEach(w => (w.type === a.attributes[attr].value ? w.script() : null)))
    }
  })
}
