import { handleEvents, ajaxLoad, matchRoutePattern } from 'helpers'
...
const replaceInnerHTML = (url, data, title, headline, push = false) => {
  const showPage = matchRoutePattern(url)
  ...
  if (showPage) {
    const breadcrumbs = document.querySelectorAll(
      '[class$="--appeared"] .at_menu__breadcrumb.breadcrumb .breadcrumb__show',
    )
    breadcrumbs.forEach(el => el.classList.add('breadcrumb__show--visible'))
    document
