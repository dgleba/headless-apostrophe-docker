...
const replaceInnerHTML = (url, data, push = false) => {
  ...
  const indexContent = data.indexOf(
    '<div class="apos-refreshable" data-apos-refreshable>'
  )
  document.title =
    indexTitle > -1
      ? data.substring(indexTitle + 7, data.indexOf('</title>'))
      : setNewTitle() // no index title in homepage ajax, so need to set it through setNewTitle
  aposRefreshable.innerHTML =
    indexContent > -1
      ? data.substring(indexContent + 52, data.indexOf('</main>'))
      : data // no "apos-refreshable" div in homepage ajax
  handleEvents()
}
...
const setNewTitle = () => {
  ...
  return array.pop().push(title).join(' | ', true) // in order to chainify calls
}
