...
const enterArticle = () => {
  ...
  if (list) {
    ...
    tl
      ...
      .to(list, 1.5, { opacity: 1, paddingTop: '20px' })
      ...
      if (url === '/') {
        ...
        tl
          .staggerTo(
            '.at_article__element',
            1,
            { transform: 'translateY(-500px)', opacity: 0, force3d: true, ease: Back.easeInOut },
            0.1,
          )
          .to(list, 0.5, { opacity: 0, paddingTop: '50px' })
          .to(article, 0.8, { height: 0, ease: Power4.easeIn }, 0) // run in parallel
      }
    }
  })
