...
import animateHome from '../home/client-home.js'
...
const handleEvents = () => {
  ...
  pages.forEach(page => {
    if (window.location.pathname === page.slug) {
      if (page.type === 'home') {
        // animate menu bars and svg on homepage
        const navs = document.querySelectorAll('nav[page]')
        navs.forEach((nav, i) =>
          nav.classList.remove(`${root}__${i + 1}--disappear`)
        )
        animateHome()
      } else if (page.type === 'career-page') {
        animateCareer()
      }
    }
  })
