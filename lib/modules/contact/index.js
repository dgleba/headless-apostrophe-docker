...
module.exports = {
  ...
  construct: (self, options) => {
    self.setSubmitSchema = () => self.submitSchema = self.apos.schemas.subset(self.schema, ['email','message'])
    ...
    self.beforeInsert = (req, piece, options, callback) => {
      piece.published = true // p is a piece
      piece.title = piece.email + crypto.randomBytes(5).toString('hex').slice(0, 8)
      return callback()
    }
    ...
    self.afterInsert = (req, piece, options, callback) => {
      ...
      transporter.sendMail(mailOptions, (error, info) => error ? console.log(error) : console.log('Message sent: %s', info.messageId))
