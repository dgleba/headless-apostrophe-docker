...
;(function() {
  ...
  CKEDITOR.plugins.add('widget', {
    ...
    lang:
      'af,ar,az,bg,ca,cs,cy,da,de,de-ch,el,en,en-au,en-gb,eo,es,es-mx,eu,fa,fi,fr,gl,he,hr,hu,id,it,ja,km,ko,ku,lv,nb,nl,no,oc,pl,pt,pt-br,ro,ru,sk,sl,sq,sv,tr,tt,ug,uk,vi,zh,zh-cn', // %REMOVE_LINE_CORE%
    ...
    CKEDITOR.tools.extend(
      ...
      widgetDef,
      {
        ...
        data: CKEDITOR.tools.extend(
          {},
          typeof widgetDef.defaults == 'function' ? widgetDef.defaults() : widgetDef.defaults,
        ),
  ...
  function setupNativeCutAndCopy(widgetsRepo) {
    ...
    function eventListener(evt) {
      if (widgetsRepo.focused) copySingleWidget(widgetsRepo.focused, evt.name == 'cut')
    }
  }
  ...
  function setupWidgetsLifecycleStart(widgetsRepo) {
    ...
    editor.on('dataReady', function() {
      ...
      if (snapshotLoaded) cleanUpAllWidgetElements(widgetsRepo, editor.editable())
      snapshotLoaded = 0
  ...
  function stateUpdater(widgetsRepo) {
    ...
    return {
      select: function(widget) {
        if (CKEDITOR.tools.indexOf(currentlySelected, widget) < 0) toBeSelected.push(widget)
      ...
      commit: function() {
        ...
        if (focusedChanged && (widget = widgetsRepo.focused)) blurWidget(widgetsRepo, widget)
  ...
  function setupDragHandler(widget) {
    ...
    if (container) img = container.findOne('img')
      ...
      container.setAttributes({
        ...
        style:
          'background:rgba(220,220,220,0.5);background-image:url(' + editor.plugins.widget.path + 'images/handle.png)',
      })
  ...
  function setupWidgetData(widget, startupData) {
    ...
    if (widgetDataAttr) widget.setData(JSON.parse(decodeURIComponent(widgetDataAttr)))
  ...
  function addCustomStyleHandler() {
    ...
    CKEDITOR.style.addCustomHandler({
      ...
      remove: function(editor) {
        ...
        if (this.checkApplicable(editor.elementPath(), editor)) editor.widgets.focused.removeStyle(this)
      },
      ...
      toAllowedContentRules: function(editor) {
        ...
        if (widgetDef.styleToAllowedContentRules) return widgetDef.styleToAllowedContentRules(this)
        return null
      },
