...
;(function() {
  ...
  CKEDITOR.plugins.add('codesnippet', {
    ...
    lang:
      'ar,az,bg,ca,cs,da,de,de-ch,el,en,en-au,en-gb,eo,es,es-mx,et,eu,fa,fi,fr,fr-ca,gl,he,hr,hu,id,it,ja,km,ko,ku,lt,lv,nb,nl,no,oc,pl,pt,pt-br,ro,ru,sk,sl,sq,sv,th,tr,tt,ug,uk,vi,zh,zh-cn', // %REMOVE_LINE_CORE%
    ...
    beforeInit: function(editor) {
      ...
      this.setHighlighter = function(highlighter) {
        ...
        var langs = (editor._.codesnippet.langs = editor.config.codeSnippet_languages || highlighter.languages)
        ...
        editor._.codesnippet.langsRegex = new RegExp(
          '(?:^|\\s)language-(' + CKEDITOR.tools.objectKeys(langs).join('|') + ')(?:\\s|$)',
        )
      }
    },
      ...
      if (!editor._.codesnippet.highlighter) {
        var hljsHighlighter = new CKEDITOR.plugins.codesnippet.highlighter({
          ...
          highlighter: function(code, language, callback) {
            var highlighted = this.hljs.highlightAuto(code, this.hljs.getLanguage(language) ? [language] : undefined)
  ...
  function registerWidget(editor) {
    ...
    editor.widgets.add('codeSnippet', {
      ...
      highlight: function() {
        var that = this,
          ...
          callback = function(formatted) {
            ...
            that.parts.code.setHtml(isBrowserSupported ? formatted : formatted.replace(newLineRegex, '<br>'))
          }
      ...
      data: function() {
        ...
        if (newData.code) this.parts.code.setHtml(CKEDITOR.tools.htmlEncode(newData.code))

        // Remove old .language-* class.
        if (oldData && newData.lang != oldData.lang) this.parts.code.removeClass('language-' + oldData.lang)
      ...
      upcast: function(el, data) {
        ...
        if (childrenArray.length != 1 || (code = childrenArray[0]).name != 'code') return

        // Upcast <code> with text only: https://dev.ckeditor.com/ticket/11926#comment:4
        if (code.children.length != 1 || code.children[0].type != CKEDITOR.NODE_TEXT) return
    ...
    function getNonEmptyChildren(parentElement) {
      ...
      for (var i = preChildrenList.length - 1; i >= 0; i--) {
        ...
        if (curNode.type != CKEDITOR.NODE_TEXT || !curNode.value.match(whitespaceOnlyRegex)) ret.push(curNode)
      }
